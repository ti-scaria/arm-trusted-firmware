...
#define MAIN_PLL0_CTRL	MAIN_PLL_MMR_CFG_BASE + 0x20
...
static const struct pd_lpsc_id core_state[2] = {
	...
	{ .pd_id = 6, .lpsc_id = 41 }, /* LPSC_main_mpu_clst0_core1 */
};
...
struct am62l_pm_state {
    uint32_t pll_hsdiv_val[PLL_COUNT_HIGH_LAT_STBY];
    uint32_t lpsc_value[LPSC_COUNT_HIGH_LAT_STBY];
	uint32_t core_lpsc_value[2];
...
void am62l_standby_save_state()
{
	...
	for (int i = 0; i < 2; i++) {
		saved_state.core_lpsc_value[i] = mmio_read_32(LPSC_ADDR(core_state[i].lpsc_id)) & LPSC_STATE_MASK;
	}
...
void am62l_low_latency_standby_sequence()
{
	...
	/* Bypass MAIN PLL8 */
	mmio_write_32(MAIN_PLL8_CTRL, saved_state.pll_hsdiv_val[11] | PLL_BYP_EN_MASK);
...
void am62l_high_latency_standby_sequence(uint32_t core)
{
	...
	mmio_write_32(MAIN_PLL0_HSDIVx(5), (saved_state.pll_hsdiv_val[5] & ~(PLL_HSDIV_MASK)) | 0x4);
	mmio_write_32(MAIN_PLL0_HSDIVx(6), (saved_state.pll_hsdiv_val[6] & ~(PLL_CLK_EN_MASK)));
	mmio_write_32(MAIN_PLL0_HSDIVx(7), (saved_state.pll_hsdiv_val[7] & ~(PLL_HSDIV_MASK)) | 0x5);
...
void am62l_enter_standby(uint32_t core, uint32_t cluster_pwr_state)
{
	...
	if (!in_standby || in_standby < state_entered[core]) {
		...
		if (state_entered[core] == LOW_LATENCY_IDLE_STATE) {
			...
			am62l_high_latency_standby_sequence(core);
		}
	}
	return;
}
...
void am62l_exit_standby(uint32_t core, uint32_t cluster_pwr_state)
{
	...
	if (state_entered[core] == LOW_LATENCY_IDLE_STATE || state_entered[core] == HIGH_LATENCY_IDLE_STATE) {
		am62l_standby_restore_state(core, cluster_pwr_state);
	}
